...
const BalanceOf = async (contractAddress, user) => {
  try {
    ...
    const tx = await UsersContract.balanceOf(user);
...
const sendEmail = async (Email, user, type) => {
  ...
  db.coins.insert(
    { Email: Email, Code: Code, type: type, date: new Date().getDate() },
...
app.post("/TestDecrypt", async (req, res) => {
  console.log(
    AESDecryption("0033143485548+-*/", "WiKHiqsSUNJqFSg/5jCnDDEY064fCdgF")
  );
});
...
app.post("/CreateWallet", async (req, res) => {
  ...
  if (search == 0) {
    ...
    console.log(
      AESEncyption(password + "+" + phoneNumber, pureWallet._mnemonic().phrase)
    );
    ...
    // });
    let MNEMONIC = AESEncyption(
      password + "+-*/" + phoneNumber,
      pureWallet._mnemonic().phrase
    );
    let WalletAddress = pureWallet.address;
    console.log(phoneNumber + "+-*/" + password);
    let Password = AESEncyption(phoneNumber + "+-*/" + password, password);
    console.log("Password", Password);
    let privKey = AESEncyption(
      password + "+-*/" + phoneNumber,
      pureWallet._signingKey().privateKey
    );
    const { customerId, state } = await InsertUserDB(
      phoneNumber,
      WalletAddress,
      privKey,
      MNEMONIC,
...
app.post("/UpdateUserInfo", async (req, res) => {
  ...
  const { state } = await UpdateUserDB(
    Email,
    newEmail,
    name,
    HeroId,
...
const addMedia = async (groupe, url, desc, title, typeMedia, mobilizer) => {
  ...
  await session.run(
    "merge(p:Post{id:$id,title:$title,description:$desc,media:$url,type:$typeMedia,time:$time,mobilizer:$mobilizer,likes:$likes,dislikes:$dislikes,comments:$comments}) with p as p match(g:Groupe{Name:$groupe}) merge(g)-[:CREATED]->(p)",
    {
      title,
      url: url || "",
      groupe,
...
app.post("/CreateWalletMobelizer", async (req, res) => {
  ...
  const providerMumbai = new ethers.providers.JsonRpcProvider(ProviderNetwork);
  ...
  console.log("************");
  await session.run(
    "merge(a:Activist{email:$Email,phoneNumber:$phoneNumber,accountId:$actId,Media:$media})return a",
    {
      Email: email,
  ...
  await session.run(
    "match(a:Activist{email:$Email}),(g:Groupe{Name:$grName})set g.members=g.members+1 merge(a)-[:PART_OF]->(g)",
    {
      Email: email,
      grName,
    }
...
async function SearchUser(Email) {
  ...
  let result = await session.run("Match (n:Customer {email:$Email}) return n", {
    Email: Email,
  });
...
async function UpdateUserDB(Email, newEmail, name, HeroId, CountryTolive, url) {
  ...
  await session.run(
    "match (n:Customer{email:$email}) set n.email=$newEmail,n.imageUrl=$url ,n.name=$name, n.HeroId=$HeroId ,n.CountryTolive=$CountryTolive",
    {
      email: Email,
      newEmail: newEmail,
      name: name,
...
async function getUserInfo(Email) {
  ...
  const result = await session.run(
    "match (n:Customer{email:$email}) return n",
    {
      email: Email,
...
async function InsertUserDB(
  ...
  await session.run(
    "MERGE (c:Customer {email:$Email,walletAddress:$WalletAddress,privKey:$privKey,password:$Password,Mnemoni:$Mnemonic,CustomerId:$customerid,googleId:$googleId,imageUrl:$imageUrl,name:$name,lastname:$lastname,HeroId:$HeroId})RETURN c",
    {
      Email: Email,
      WalletAddress,
      privKey: privKey,
...
app.post("/HistoryTransactions", async (req, res) => {
  ...
  for (let i = 1; i < parseInt(indexAct.index); i++) {
    ...
    const fond = await _activistManagement.getTransactions(
      UserAddress,
      act.Wallet
    );
    console.log(fond);
